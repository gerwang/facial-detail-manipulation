...
def extract_id_name_exp_name(file_path):
    id_name = file_path.split('/')[-3]
    exp_name = os.path.splitext(os.path.basename(file_path))[0]
    return id_name, exp_name


def interleave(x, y):
    return torch.cat([x[None, ...], y[None, ...]]).transpose(0, 1).reshape((-1,) + x.size()[1:])
...
def has_arg(parser, arg):
    for x in parser._actions:
        if arg in x.option_strings:
            return True
    return False


def load_clean_data_set(clean_data_path):
    res = set()
    with open(clean_data_path) as f:
        for line in f.readlines():
            id_name, exp_name = line.strip().split()
            res.add((id_name, exp_name))
    return res
