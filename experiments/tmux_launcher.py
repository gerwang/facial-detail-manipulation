...
class TmuxLauncher():
    ...
    def demo_commands(self):
        opts = self.test_options()
        return ["QT_AUTO_SCREEN_SCALE_FACTOR=1 python demo.py " + str(opt) for opt in opts]

    def exp_age_edit_commands(self):
        opts = self.exp_age_edit_options()
        return ["python test.py " + str(opt) for opt in opts]

    def bs_anime_commands(self):
        opts = self.bs_anime_options()
        return ["python test.py " + str(opt) for opt in opts]

    def exp_inter_commands(self):
        opts = self.exp_inter_options()
        return ["python test.py " + str(opt) for opt in opts]

    def age_progression_commands(self):
        opts = self.age_progression_options()
        return ["python test.py " + str(opt) for opt in opts]
