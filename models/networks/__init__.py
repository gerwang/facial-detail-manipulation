...
def modify_commandline_options(parser, is_train):
    ...
    if opt.netTExp is not None:
        netTExp_cls = find_network_using_name(opt.netTExp, 'transformer')
        assert netTExp_cls is not None
        parser = netTExp_cls.modify_commandline_options(parser, is_train)

    if opt.netTAge is not None:
        netTAge_cls = find_network_using_name(opt.netTAge, 'transformer')
        assert netTAge_cls is not None
        parser = netTAge_cls.modify_commandline_options(parser, is_train)
