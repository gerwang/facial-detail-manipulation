...
class StyleGAN2ResnetEncoder(BaseNetwork):
    ...
    def modify_commandline_options(parser, is_train):
        ...
        parser.add_argument('--netE_start_nc', type=int, default=0)
        parser.add_argument("--netE_nc_max", type=int, default=-1, help='-1 means no restriction')
        return parser
    ...
    def __init__(self, opt):
        ...
        self.add_module("FromRGB", ConvLayer(opt.input_nc - opt.netE_start_nc, self.nc(0), 1))
    ...
    def nc(self, idx):
        ...
        if self.opt.netE_nc_max != -1:
            nc = min(self.opt.netE_nc_max, int(round(nc)))
        # nc = min(self.opt.global_code_ch, int(round(nc)))
        return round(nc)

    def forward(self, x, extract_features=False):
        x = x[:, self.opt.netE_start_nc:]
        x = self.FromRGB(x)
...
class StructureStyleSeparateEncoder(BaseNetwork):
    @staticmethod
    def modify_commandline_options(parser, is_train):
        parser.add_argument("--netE_scale_capacity", default=1.0, type=float)
        parser.add_argument("--netE_num_downsampling_sp", default=4, type=int)
        ...
        parser.add_argument("--netE_nc_steepness", default=2.0, type=float)
        parser.add_argument('--netE_start_nc', type=int, default=0)
        parser.add_argument('--netE_gl_channel', type=int, default=1)  # dpmap
        parser.add_argument('--netE_sp_channel', type=int, default=0)  # seg
        parser.add_argument("--netE_nc_max", type=int, default=-1, help='-1 means no restriction')
        return parser
    ...
    def __init__(self, opt):
        ...
        self.add_module("FromRGBSp", ConvLayer((opt.input_nc - opt.netE_start_nc) // 2, self.nc(0), 1))
        self.add_module("FromRGBGl", ConvLayer((opt.input_nc - opt.netE_start_nc) // 2, self.nc(0), 1))

        self.DownToSpatialCodeSp = nn.Sequential()
        for i in range(self.opt.netE_num_downsampling_sp):
            self.DownToSpatialCodeSp.add_module(
                "ResBlockDownBy%d" % (2 ** i),
                ResBlock(self.nc(i), self.nc(i + 1), blur_kernel,
                         reflection_pad=True)
            )

        self.DownToSpatialCodeGl = nn.Sequential()
        for i in range(self.opt.netE_num_downsampling_sp):
            self.DownToSpatialCodeGl.add_module(
                "ResBlockDownBy%d" % (2 ** i),
                ResBlock(self.nc(i), self.nc(i + 1), blur_kernel,
                         reflection_pad=True)
            )
    ...
    def forward(self, x, extract_features=False):
        x = x[:, self.opt.netE_start_nc:]
        x_sp = x[:, self.opt.netE_sp_channel:self.opt.netE_sp_channel + 1]
        x_gl = x[:, self.opt.netE_gl_channel:self.opt.netE_gl_channel + 1]
        x_sp = self.FromRGBSp(x_sp)
        x_gl = self.FromRGBGl(x_gl)
        midpoint_sp = self.DownToSpatialCodeSp(x_sp)
        midpoint_gl = self.DownToSpatialCodeGl(x_gl)
        sp = self.ToSpatialCode(midpoint_sp)

        if extract_features:
            padded_midpoint = F.pad(midpoint_sp, (1, 0, 1, 0), mode='reflect')
            feature = self.DownToGlobalCode[0](padded_midpoint)
            assert feature.size(2) == sp.size(2) // 2 and \
                   feature.size(3) == sp.size(3) // 2
            feature = F.interpolate(
        ...
        x = self.DownToGlobalCode(midpoint_gl)
        x = x.mean(dim=(2, 3))
        gl = self.ToGlobalCode(x)
        sp = util.normalize(sp)
        gl = util.normalize(gl)
...
class BothStyleEncoder(BaseNetwork):
    @staticmethod
    def modify_commandline_options(parser, is_train):
        parser.add_argument("--netE_scale_capacity", default=1.0, type=float)
        parser.add_argument("--netE_num_downsampling_sp", default=4, type=int)
    ...
    def __init__(self, opt):
        ...
        self.add_module("FromRGB", ConvLayer(opt.input_nc, self.nc(0), 1))
        ...
        self.add_module(
            "ToGlobalCode",
            nn.Sequential(
                EqualLinear(nchannels, self.opt.spatial_code_ch + self.opt.global_code_ch)
            )
        )
    ...
    def forward(self, x):
        ...
        gl = self.ToGlobalCode(x)
        gl = util.normalize(gl)
        sp, gl = torch.split(gl, [self.opt.spatial_code_ch, self.opt.global_code_ch], dim=1)
        return sp, gl
