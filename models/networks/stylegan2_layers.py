...
class ToRGB(nn.Module):
    def __init__(self, in_channel, style_dim, upsample=True, blur_kernel=[1, 3, 3, 1], input_nc=3):
        super().__init__()

        if upsample:
            self.upsample = Upsample(blur_kernel)

        self.conv = ModulatedConv2d(in_channel, input_nc, 1, style_dim, demodulate=False)
        self.bias = nn.Parameter(torch.zeros(1, input_nc, 1, 1))
...
class ResBlock(nn.Module):
    def __init__(self, in_channel, out_channel, blur_kernel=[1, 3, 3, 1], reflection_pad=False, pad=None,
                 downsample=True):
        super().__init__()
        ...
        self.conv2 = ConvLayer(in_channel, out_channel, 3, downsample=downsample, blur_kernel=blur_kernel,
                               reflection_pad=reflection_pad, pad=pad)
...
class FeatureMatchingLoss:
    ...
    def __call__(self, input: list, target: list):
        res = [self.l1_loss(x, y) for x, y in zip(input, target)]
        return sum(res) / len(res)


class Discriminator(nn.Module):
    def __init__(self, size, channel_multiplier=2, blur_kernel=[1, 3, 3, 1], input_nc=3, output_nc=1):
        super().__init__()
        ...
        convs = [('0', ConvLayer(input_nc, channels[size], 1))]
        ...
        self.final_linear = nn.Sequential(
            ...
            EqualLinear(channels[4], output_nc),
        )

    def forward(self, input, extract_features=False):
        if extract_features:
            feat = [input]
            for module in self.convs.children():
                feat.append(module(feat[-1]))
            out = feat[-1]
            feat = feat[1:]
        else:
