...
class SwappingAutoencoderModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train):
        ...
        parser.add_argument('--lambda_LPIPS', default=0.0, type=float)
        parser.add_argument('--lpips_channel_index', type=int, default=0)
    ...
    def compute_lpips(self, input, target):
        return self.lpips_loss(input[:, self.opt.lpips_channel_index][:, None, ...].repeat((1, 3, 1, 1)),
                        target[:, self.opt.lpips_channel_index][:, None, ...].repeat((1, 3, 1, 1)))
    ...
    def compute_generator_losses(self, real, sp_ma, gl_ma):
        ...
        metrics["LPIPS_dist"] = self.lpips_loss(rec[:, self.opt.lpips_channel_index][:, None, ...].repeat((1, 3, 1, 1)),
                                                real[:B // 2, self.opt.lpips_channel_index][:, None, ...].repeat(
                                                    (1, 3, 1, 1)))
